...
class GeminiNativeModel(Model):
    ...
    def __init__(self, model: str = "gemini-2.5-flash-preview-04-17", batch_size: int = 0, thinking_budget: int = 0, rpm: int = 10, **kwargs):
        """
    ...
    async def stream_abatch(self, prompts: List[Union[str, List[Dict[str, str]]]], batch_size: int = None) -> AsyncGenerator[dict, None]:
        ...
        while len(pending_tasks) < batch_size and next_prompt_index < len(prompts):
            task = asyncio.create_task(self.ainvoke(prompts[next_prompt_index]))
        ...
        while pending_tasks:
            ...
            for task in done:
                ...
                if next_prompt_index < len(prompts):
                    new_task = asyncio.create_task(self.ainvoke(prompts[next_prompt_index]))
